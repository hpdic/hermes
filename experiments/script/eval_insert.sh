...
if [[ "$PREFIX" == "bitcoin" ]]; then
  ...
  mysql -u "$MYSQL_USER" -D "$MYSQL_DB" -e "
    INSERT INTO $PACK_TABLE(group_id, ctxt_repr)
    SELECT 1, HERMES_PACK_CONVERT(CAST(ROUND(value / 24) AS UNSIGNED))
    FROM $TABLE WHERE group_id = 1;"
else
  mysql -u "$MYSQL_USER" -D "$MYSQL_DB" -e "
    INSERT INTO $SINGULAR_TABLE(value, ctxt_repr)
  ...
  mysql -u "$MYSQL_USER" -D "$MYSQL_DB" -e "
    INSERT INTO $PACK_TABLE(group_id, ctxt_repr)
    SELECT 1, HERMES_PACK_CONVERT(value)
    FROM $TABLE WHERE group_id = 1;"
fi
